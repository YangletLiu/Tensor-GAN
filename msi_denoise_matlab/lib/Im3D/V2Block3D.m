...
TotalPatNum = (floor((size(Video,1)-patsize)/step)+1)*(floor((size(Video,2)-patsize)/step)+1)*(floor((size(Video,3)-patsize)/step)+1);                  %Total Patch Number in the image
Z           =   zeros(patsize,patsize, patsize, TotalPatNum);                                       %Patches in the original noisy image
